...
const ManagePharmacyStaff = () => {
  ...
  const { data: staffMembers, create, remove, update } = usePharmacy();
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast({ title: "Staff Removed", description: "Record deleted successfully." });
  ...
  const handleAddStaff = async () => {
    ...
    try {
      ...
      toast({ title: "Staff Added", description: `${form.name} registered successfully.` });
      setShowModal(false);
      resetForm();
    } catch (error: any) {
  ...
  return (
    <DashboardLayout sidebarItems={sidebarItems} role="Admin" roleBadgeClass="badge-admin" userName="Dr. Admin">
      <div className="space-y-6">
        ...
        <DetailModal open={showModal} onClose={() => { setShowModal(false); resetForm(); }} title={selectedStaff ? "Edit Pharmacy Staff" : "Add Pharmacy Staff"}>
